  ...
  return (
    <div className="pb-16 pt-2">
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {language === 'kk' ? 'Тапсырыстар мен шығындар тарихын басқару' : 'Управление заказами и историей расходов'}
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-6 scrollbar-none">
        {[
          { id: 'overview', label: t.dashboard.overviewTab },
          { id: 'orders', label: `${t.dashboard.ordersTab} (${orders.length})` },
          { id: 'profile', label: t.dashboard.profileTab },
        ].map((tab) => {
      ...
      <main className="w-full space-y-6">
          ...
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-gray-50 dark:bg-[#111111] p-6 rounded-3xl space-y-2">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs text-gray-400 font-medium">
                    {language === 'kk' ? 'Барлық жұмсалған сома' : 'Всего потрачено'}
                  </span>
                ...
                <span className="text-[11px] text-gray-400 mt-1 block">
                  {language === 'kk' ? 'Барлық сатып алулар бойынша' : 'По всем покупкам'}
                </span>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-[#111111] p-6 rounded-3xl space-y-2">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs text-gray-400 font-medium">
                    {language === 'kk' ? 'Орындалған тапсырыстар' : 'Покупок оформлено'}
                  </span>
                ...
                <h3 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">
                  {ordersCount} {language === 'kk' ? 'тапсырыс' : 'заказов'}
                </h3>
                <span className="text-[11px] text-gray-400 mt-1 block">
                  {language === 'kk' ? 'Жылдам берілу' : 'Мгновенная выдача'}
                </span>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-[#111111] p-6 rounded-3xl space-y-2">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs text-gray-400 font-medium">
                    {language === 'kk' ? 'Орташа чек' : 'Средний чек'}
                  </span>
                  <div className="p-2 bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-white rounded-xl shadow-xs">
                ...
                <span className="text-[11px] text-gray-400 mt-1 block">
                  {language === 'kk' ? 'Бір тапсырысқа шаққанда' : 'На один заказ'}
                </span>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-black text-lg text-gray-900 dark:text-white">
                    {language === 'kk' ? 'Соңғы тапсырыстар' : 'Последние заказы'}
                  </h3>
                  <p className="text-xs text-gray-400">
                    {language === 'kk' ? 'Электрондық чектерге жылдам қолжетімділік' : 'Быстрый доступ к электронным чекам'}
                  </p>
                </div>
                {orders.length > 0 && (
                  <button
                  ...
                  >
                    {language === 'kk' ? 'Барлығын көру' : 'Смотреть все'} ({orders.length})
                  </button>
                )}
              </div>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">{t.dashboard.ordersTab}</h2>
                <p className="text-xs text-gray-400">
                  {language === 'kk' ? 'Сатып алулар тізімі және чектерді басып шығару' : 'Детализация покупок и печать чеков'}
                </p>
              </div>
              <span className="text-xs font-bold text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[#1a1a1a] px-3 py-1 rounded-full">
                {language === 'kk' ? 'Барлығы:' : 'Всего:'} {orders.length}
              </span>
            </div>
                  ...
                  >
                    ...
                    <div className="text-[11px] text-gray-400 flex flex-col sm:flex-row sm:items-center justify-between pt-1 gap-2">
                      ...
                      <span className="uppercase font-bold text-gray-500">
                        {language === 'kk' ? 'Төлем' : 'Оплата'}: {order.paymentMethod.replace('_', ' ')}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'profile' && (
          <div className="bg-gray-50 dark:bg-[#111111] rounded-3xl p-6 sm:p-8 space-y-6 animate-in fade-in duration-200 text-xs">
            <h2 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">{t.dashboard.profileTab}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white dark:bg-[#161616] p-5 rounded-2xl shadow-xs space-y-1">
                <span className="text-[11px] text-gray-400 font-semibold block">
                  {language === 'kk' ? 'Пайдаланушы аты' : 'ФИО Клиента'}
                </span>
                <p className="font-bold text-sm text-gray-900 dark:text-white">{user?.name || t.dashboard.clientRole}</p>
              </div>

              <div className="bg-white dark:bg-[#161616] p-5 rounded-2xl shadow-xs space-y-1">
                <span className="text-[11px] text-gray-400 font-semibold block">{t.auth.emailLabel}</span>
                <p className="font-bold text-sm text-gray-900 dark:text-white">{user?.email || (language === 'kk' ? 'Көрсетілмеген' : 'Не указан')}</p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#161616] p-5 rounded-2xl shadow-xs space-y-3">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {language === 'kk' ? 'Алматы қ., Абай даңғылы 150, Б блогы' : 'г. Алматы, пр. Абая 150, блок Б'}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-[#161616] p-5 rounded-2xl shadow-xs space-y-3">
              <div className="flex items-center gap-2 font-bold text-gray-900 dark:text-white text-sm">
                ...
                <span>{language === 'kk' ? 'Қауіпсіздік және Kaspi Pay' : 'Безопасность и Kaspi Pay'}</span>
              </div>
              <p className="text-gray-500 dark:text-gray-400 text-[11px] leading-relaxed">
                {language === 'kk'
                  ? 'Барлық төлемдер Kaspi қауіпсіздік хаттамасымен қорғалған. Қаражат тек тапсырыс расталған кезде есептен шығарылады.'
      ...
      {selectedReceiptOrder && (
        <Modal
          ...
          title={`${language === 'kk' ? 'Электрондық чек' : 'Электронный чек'} #${selectedReceiptOrder.id}`}
          maxWidth="max-w-md"
        >
          <div className="space-y-4 text-xs">
            <div className="pb-3 text-center">
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">{language === 'kk' ? '«KitapAll Marketplace» ЖШС' : 'ТОО «KitapAll Marketplace»'}</p>
              <p className="text-[11px] text-gray-400">{t.dashboard.orderDate(selectedReceiptOrder.createdAt)}</p>
            </div>
              ...
              {selectedReceiptOrder.items.map((item, idx) => (
                <div key={idx} className="flex justify-between py-1">
                  <div>
                    ...
                    <p className="text-gray-400 text-[10px]">{item.quantity} {t.common.units} × {item.price.toLocaleString()} ₸</p>
                  </div>
                  <span className="font-bold text-gray-900 dark:text-white">
                    {(item.price * item.quantity).toLocaleString()} ₸
                  </span>
            ...
            <div className="pt-2 space-y-1.5 px-1">
              <div className="flex justify-between font-bold text-sm">
                <span className="text-gray-700 dark:text-gray-300">{language === 'kk' ? 'Барлығы төленді:' : 'Итого оплачено:'}</span>
              ...
              <div className="flex justify-between text-gray-500 dark:text-gray-400 text-[11px]">
                ...
                <span className="font-semibold text-gray-900 dark:text-white">{language === 'kk' ? 'Төленді және берілді' : 'Оплачено и выдано'}</span>
              </div>
            </div>
            ...
            <div className="pt-3 flex gap-2">
              ...
              >
                ...
                <span>{language === 'kk' ? 'Басып шығару' : 'Распечатать'}</span>
              </button>
              <button
