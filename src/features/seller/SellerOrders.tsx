...
export const SellerOrders: React.FC = () => {
  ...
  const handleStatusChange = (orderId: string, newStatus: SellerOrderStatus, statusLabel: string) => {
    ...
    addToast({
      message: `Заказ #${orderId} переведен в статус «${statusLabel}»`,
      type: 'success',
    });
  };
      ...
      case 'new':
        return (
          <span className="inline-flex items-center gap-1.5 bg-[#FFF1F0] dark:bg-[#F14635]/20 text-[#F14635] text-xs font-bold px-3 py-1 rounded-xl">
            <Clock className="w-3.5 h-3.5" /> Новый заказ
          </span>
        );
      case 'processing':
        return (
          <span className="inline-flex items-center gap-1.5 bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 text-xs font-bold px-3 py-1 rounded-xl">
            <Boxes className="w-3.5 h-3.5" /> В сборке
          </span>
        );
      case 'shipped':
        return (
          <span className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 text-xs font-bold px-3 py-1 rounded-xl">
            <Truck className="w-3.5 h-3.5" /> В пути курьером
          </span>
        );
      case 'delivered':
        return (
          <span className="inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 text-xs font-bold px-3 py-1 rounded-xl">
            <PackageCheck className="w-3.5 h-3.5" /> Доставлен покупателю
          </span>
        );
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Всего заказов: <span className="font-bold text-gray-900 dark:text-white">{orders.length}</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-[#111111] p-4 rounded-3xl space-y-3">
        ...
        <div className="flex items-center gap-2 pt-1 overflow-x-auto">
          {[
            ...
            { id: 'processing', label: `В сборке (${orders.filter((o) => o.status === 'processing').length})` },
            { id: 'shipped', label: `В пути (${orders.filter((o) => o.status === 'shipped').length})` },
            { id: 'delivered', label: `Доставлены (${orders.filter((o) => o.status === 'delivered').length})` },
          ].map((tab) => (
            <button
              key={tab.id}
              type="button"
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          <div className="bg-white dark:bg-[#111111] rounded-3xl p-12 text-center space-y-3 border border-gray-100 dark:border-neutral-800/80">
            ...
            <h4 className="font-bold text-sm text-gray-900 dark:text-white">Заказы не найдены</h4>
            <p className="text-xs text-gray-400">В этой категории заказов пока нет</p>
          </div>
        ) : (
            ...
            >
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <span className="text-[11px] text-gray-400">
                            Размер: {item.size || 'One Size'} · {item.quantity} шт.
                          </span>
                        </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-gray-50 dark:border-neutral-800/60">
                <div className="flex items-baseline gap-2">
                  <span className="text-xs text-gray-400 font-medium">Итого к получению:</span>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {order.status === 'new' && (
                    <button
                      type="button"
                      onClick={() => handleStatusChange(order.id, 'processing', 'В сборке')}
                  ...
                  {order.status === 'processing' && (
                    <button
                      type="button"
                      onClick={() => handleStatusChange(order.id, 'shipped', 'В пути')}
                  ...
                  {order.status === 'shipped' && (
                    <button
                      type="button"
                      onClick={() => handleStatusChange(order.id, 'delivered', 'Доставлен')}
                    ...
                    >
                      ...
                      <span>Подтвердить вручение</span>
                    </button>
                  )}

                  {order.status === 'delivered' && (
                    <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                      <CheckCircle2 className="w-4 h-4" /> Заказ успешно закрыт
                    </span>
                  )}
                </div>
              </div>
